...
"""
Visualization of the time courses that were simulated with the model based on the
estimated parameters from the entire ensemble set to initial concentrations
of the baseline experiment.

1. Load random parameter sampling result.
2. Import simulation results for each parameter set.
3. Create subplots for each model species that contain all different trajectories
   (100 fits -> 100 curves in each plot).
"""
...
# for each species we want the average concentration per time point across all simulation result
# data frames in the list sim_results
...
# set multiplot size and define subplots for all species which each containing all trajectories from all
# simulations (10 relevant species: P, ATP, GTP, Fuc1P, GDP, Fuc, GDP_Fucose, PP, ADP, GMP)
# 10 plots: 2 rows, 5 cols of subplots
...
for i in range(2):
    for j in range(5):
        ...
        # if condition setup allows different rendering of subplots of species with
        # exp. data compared to those where no exp. data is avilable
