...
"""
Visualization of the ensemble of estimated parameters with
a multiplot of histograms (5 x 6 plots).
"""
...
# create list of literature values (they were used as start values of the fits;
# if no literature value was available 0.1 was used as start value as it is
# the COPASI default parameter value); same order as columns in fits_data data frame;
# 0 means no lit. value was available:
# [(FKP (fucokinase)).kcat_F,(FKP (fucokinase)).Km_Fuc,(FKP (fucokinase)).Km_ATP,
# (FKP (fucokinase)).Km_Fuc1P,(FKP (fucokinase)).Km_ADP, (FKP (guanylyltransferase)).kcat_F,
# (FKP (guanylyltransferase)).Km_Fuc1P,(FKP (guanylyltransferase)).Km_GTP,
# (FKP (guanylyltransferase)).Km_GDP_Fuc,(FKP (guanylyltransferase)).Km_PP,
# (FKP (guanylyltransferase)).ki_PP, (PPA).kcat_F,(PPA).Km_PP,(GMPK).kcat_F,(GMPK).Km_GMP,
# (GMPK).Km_ATP,(GMPK).Km_GDP,(GMPK).Km_ADP,(PPK3_G).kcat_F,(PPK3_G).Km_GDP,
# (PPK3_G).Km_GTP,(PPK3_G).K_eq_PPK3_G,(PPK3_A).kcat_F,(PPK3_A).Km_ADP,(PPK3_A).Km_ATP,
# (PPK3_A).K_eq_PPK3_A,obj_val]; last 0 entry is for obj. value plot
lit_vals = [1404, 0.045, 1.08, 0, 0, 36252, 0.03295, 0.012, 0.12, 0.135, 0, 558000, 
            0.003, 752400, 0.156, 0.5, 0.097, 0.017, 15048, 1.2, 0, 1, 18780, 2.5, 2, 1, 0]
...
# 'log' -> log10(x-axis values), 'binNum' and 'binSize' set to 0 as
# placeholder value (these options are currently not used for plotting)
...
for i in range(5):
    for j in range(6):
        ...
        if n <= len(fits_data.columns)-2:
            ...
            # plot estimated parameter values as histogram
            ...
            # change notation of x axis tick labels to scientific 
            #(exponential) notation when the numbers are outsite [1e-5,1e5]
            # AND subplot x-axis is NOT log'ed!
