...
pickle_file = open("GDP-Fucose_v7XGSK_PE9XGSK_Opt7b_SlctBestOptAllParams_EvoStrat_100x_score_df.pkl", "rb")
...
ax[0].text(-0.15, 10, string.ascii_uppercase[0], transform=ax[0].transAxes, 
           size=20, weight='bold')
# 2) TITER HEATMAP: visualize pandas data frame df as heat map 
# set color map (reverse default color scheme: higher values => darker color)
cmap = sns.cm.mako_r
# specify color bar options
cax = inset_axes(ax[1],
...
N = len(labels)
wanted_label = best_total_score_index   # 'O53'
...
# OUTPUT
# save plot as .pdf
plt.savefig("GDP-Fucose_v7XGSK_PE9XGSK_EvoStrat100x_Opt7b_TiterHeatmap_fig.pdf", bbox_inches='tight')
# save plot as .png (for Word)
plt.savefig("GDP-Fucose_v7XGSK_PE9XGSK_EvoStrat100x_Opt7b_TiterHeatmap_fig.png", bbox_inches='tight')
# save plot as .svg (for Word)
plt.savefig("GDP-Fucose_v7XGSK_PE9XGSK_EvoStrat100x_Opt7b_TiterHeatmap_fig.svg", bbox_inches='tight')